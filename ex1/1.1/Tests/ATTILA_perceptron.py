...
if __name__ == '__main__':
    ...
    #Instantiate the sign activation function and calling it
    # 1.1 Test input values
   #  sign_act = SignActivation()  # Create an instance
   #  x = np.array([-2, -0.5, 0, 0.5, 3])

   #  y = sign_act.forward(x)
   #  print("Input:", x)
   #  print("Output (forward):", y)
    
   #  # Gradient test (note: your gradient method only works for single values, not arrays)
   #  for val in x:
   #     grad = sign_act.gradient(val)
   #     print(f"Gradient at x={val}: {grad}")
   ...
   # # Test each input
   #  for x in X:
   #    activation_value = perceptron.activation(x)
   #    output_value = perceptron.predict(x)
   #    grad_value = perceptron.gradient(activation_value)
   #    print(f"Input: {x}, Activation: {activation_value}, Output: {output_value}, Gradient: {grad_value}")

   #  # Call activation, output, and predict functions on a sample input
   #  sample_input = np.array([0.5, -0.5])
   #  activation_val = perceptron.activation(sample_input)
   #  output_val = perceptron.output(activation_val)
   #  predict_val = perceptron.predict(sample_input)
   #  print(f"Sample Input: {sample_input}")
   #  print(f"Activation: {activation_val}")
   #  print(f"Output: {output_val}")
   #  print(f"Predict: {predict_val}")
...
### Considerations
# 4. The combination that converges the fastest requires 3 epochs. Possibly a larger grid search could yield better results.
# 5. They do not necessarily reach the same weights, as for a linearly separable dataset there are infinitely many hyperplanes that can separate the data.
# 6. The plot shows always a perfect decision boundary. This is a property of perceptrons for linearly separable datasets.