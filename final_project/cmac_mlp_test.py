...
BASE_SPEED = 60
K_TURN = 30
USE_CMAC = True # Set to False to use MLP only
...
# Initialize CMAC and load weights if applicable
cmac = CMAC(n_inputs=2, n_outputs=2, memory_size=2048, generalization_factor=16, learning_rate=0.1)
if USE_CMAC:
    try:
        cmac.weights = np.load(CMAC_WEIGHTS_FILE)
        print(f"Loaded pre-trained CMAC weights from {CMAC_WEIGHTS_FILE}")
    except FileNotFoundError:
        print(f"Warning: CMAC weights file not found. Starting with a blank CMAC.")

# Setup Results File
if not os.path.exists(RESULTS_FILE):
    with open(RESULTS_FILE, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['timestamp', 'controller_type', 'time_to_target', 'path_distance', 'integrated_angle_error', 'action_rmse'])

# Main Evaluation Loop
while True:
    controller_type = "MLP+CMAC" if USE_CMAC else "MLP_Only"
    prompt = input(f"\nReady for new trial. Controller: {controller_type}. Reposition robot and press Enter (or 'q' to quit)...")
    if prompt.lower() == 'q': break
    
    start_time = time.time(); total_distance = 0.0; integrated_angle_error = 0.0
    action_squared_errors = []; last_robot_pos = None; trial_running = True
    ...
    while trial_running:
        ...
        if allocentric_state is None:
            ...
            if cv2.waitKey(1) & 0xFF == ord('q'): trial_running = False; break
            continue
        ...
        cmac_correction = np.array([0.0, 0.0])
        if USE_CMAC:
            ...
            # cmac.train(egocentric_state, mlp_error) # Do not train during evaluation

        final_action = mlp_prediction + cmac_correction
        api.setSpinSpeed(-int(final_action[0]), int(final_action[1]), wheels)
        
        # Metric Calculation
        current_robot_pos = np.array(allocentric_state[:2])
        if last_robot_pos is not None: total_distance += np.linalg.norm(current_robot_pos - last_robot_pos)
        last_robot_pos = current_robot_pos
        integrated_angle_error += abs(egocentric_state[1])
        perfect_action_for_metric = calculate_expert_action(allocentric_state)
        error = perfect_action_for_metric - final_action
        action_squared_errors.append(np.sum(error**2))

        if egocentric_state[0] < STOPPING_DISTANCE:
            api.setSpinSpeed(0, 0, wheels)
            time_to_target = time.time() - start_time
            action_rmse = np.sqrt(np.mean(action_squared_errors)) if action_squared_errors else 0

            print(f"\n--- Trial Complete ---\nTime: {time_to_target:.2f}s, Path: {total_distance:.2f}, Angle Err: {integrated_angle_error:.2f}, RMSE: {action_rmse:.2f}")
            with open(RESULTS_FILE, 'a', newline='') as f:
                writer = csv.writer(f)
                writer.writerow([time.strftime("%Y-%m-%d %H:%M:%S"), controller_type, time_to_target, total_distance, integrated_angle_error, action_rmse])
            trial_running = False

        cv2.imshow("Live Feed", frame)
        if cv2.waitKey(1) & 0xFF == ord('q'): trial_running = False

# Cleanup
print("\nExiting evaluation script.")
